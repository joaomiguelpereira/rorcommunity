...
activation_key varchar(256) null,
created_at datetime null,
updated_at datetime null,
activated_at datetime null,
retrieve_pwd_key varchar(256) null,
profile_compteled int,
localization varchar(100) null,
website varchar(256) null,
public_email char(1) null,
gmap_zoom_level varchar(3) null,
gmap_lat varchar(100) null,
gmap_long varchar(100) null,
primary key(id)
) ENGINE=INNODB DEFAULT CHARSET=utf8 ROW_FORMAT=DYNAMIC;
ALTER TABLE users ADD INDEX ( screen_name );
...
constraint fk_post_votes foreign key (post_id) references posts(id),
...
constraint fk_user_tags foreign key (user_id) references users(id),
primary key(id)
) ENGINE=INNODB DEFAULT CHARSET=utf8 ROW_FORMAT=DYNAMIC;
...
label_id int not null,
post_id int not null,
constraint fk_posts_labels foreign key (post_id) references posts(id),
constraint fk_labels_labels foreign key (label_id) references labels(id),
primary key(post_id,label_id)
) ENGINE=INNODB DEFAULT CHARSET=utf8 ROW_FORMAT=DYNAMIC;
...
create table user_friends (
  ...
  constraint fk_cp_user_friends_f foreign key (friend_id) references users(id),
	constraint fk_cp_user_friends_u foreign key (user_id) references users(id),
...
DROP FUNCTION IF EXISTS count_saved_by;
...
DROP FUNCTION IF EXISTS count_labels_applied;
...
DROP FUNCTION IF EXISTS has_label;
...
DROP FUNCTION IF EXISTS has_category;
...
DROP FUNCTION IF EXISTS url_has_label;
...
DROP FUNCTION IF EXISTS last_activity;
